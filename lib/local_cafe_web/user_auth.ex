defmodule LocalCafeWeb.UserAuth do
  ...
  @remember_me_cookie "_local_cafe_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      LocalCafeWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  defp renew_session(conn, _user) do
    ...
    # Preserve cart and tip amount across login
    cart = get_session(conn, :cart)
    tip_amount = get_session(conn, :tip_amount)

    conn
    |> configure_session(renew: true)
    |> clear_session()
    |> maybe_restore_cart(cart)
    |> maybe_restore_tip(tip_amount)
  end

  defp maybe_restore_cart(conn, nil), do: conn
  defp maybe_restore_cart(conn, cart), do: put_session(conn, :cart, cart)

  defp maybe_restore_tip(conn, nil), do: conn
  defp maybe_restore_tip(conn, tip), do: put_session(conn, :tip_amount, tip)
  ...
  # For POST requests (like comment creation), use the referer header
  # to return to the page they came from after login
  defp maybe_store_return_to(conn) do
    case get_req_header(conn, "referer") do
      [referer] ->
        ...
        put_session(conn, :user_return_to, return_path)
