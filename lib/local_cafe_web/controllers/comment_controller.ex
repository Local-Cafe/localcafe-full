defmodule LocalCafeWeb.CommentController do
  ...
  def create(conn, %{"slug" => slug, "comment" => comment_params}) do
    ...
    # Require username before allowing comments
    if is_nil(user.username) || user.username == "" do
      conn
      |> put_flash(
        :error,
        "You must set a username before commenting. Please update your settings."
      )
