...
**LocalCafe** is a Phoenix 1.8 project that reimagines the Phoenix framework with a web standards-first approach. The project philosophy emphasizes semantic HTML, vanilla CSS with design tokens, and minimal JavaScript over LiveView and Tailwind defaults.
...
**Supervision Tree** (lib/local_cafe/application.ex:10-18):
...
- Development DB: `local_cafe_dev` (port 5432)
- Test DB: `local_cafe_test` (with MIX_TEST_PARTITION support)
...
5. **Use Phoenix Channels strategically**: Only for true real-time features, not as default
...
```heex
<!-- lib/local_cafe_web/components/core_components.ex -->
...
- Templates: `lib/local_cafe_web/` (.ex, .heex files)
- Translations: `priv/gettext/`

### Phoenix and Elixir Best Practices
...
- Extract reusable components to separate files in `lib/local_cafe_web/components/`
- Use semantic HTML elements as the foundation

**Ecto**:

- Write clear, explicit queries
- Use changesets for all data validation
- Leverage Ecto's composable query syntax
...
- Use pipelines for shared request processing
